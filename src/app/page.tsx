...
export default function HomePage() {
  ...
  useEffect(() => {
    fetch("/api/auth/me").then(r => r.json()).then(d => {
      setUser(d.user); return fetch("/api/fortune")
    }).then(r => r.json()).then(d => { setFortune(d) }).catch(() => {}).finally(() => setLoading(false))
  }, [])
        ...
        {fortune && (
          <div className="glass rounded-[var(--radius-lg)] p-5 mb-8 animate-fade-in-up ornament-border">
            ...
            <div className="grid grid-cols-2 gap-[10px]">
              {[
                { label: "爱情", tag: fortune.love.tag, icon: "♀" },
                { label: "事业", tag: fortune.work.tag, icon: "♂" },
                { label: "财运", tag: fortune.wealth.tag, icon: "◎" },
                { label: "心情", tag: fortune.mood.tag, icon: "☽" },
              ].map(item => (
