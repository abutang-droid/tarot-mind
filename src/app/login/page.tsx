    ...
    <div className="min-h-screen flex items-center justify-center px-5">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs mb-1.5" style={{ color: 'var(--ivory-dim)' }}>邮箱</label>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)}
              className="w-full px-4 py-3 rounded-[var(--radius-sm)] text-sm outline-none transition-all"
              style={{ background: 'rgba(12,15,35,0.8)', border: '1px solid rgba(201,149,74,0.12)', color: 'var(--ivory)' }}
              onFocus={e => e.target.style.borderColor = 'var(--gold)'}
              onBlur={e => e.target.style.borderColor = 'rgba(201,149,74,0.12)'}
              placeholder="your@email.com" required />
          </div>
          <div>
            <label className="block text-xs mb-1.5" style={{ color: 'var(--ivory-dim)' }}>密码</label>
          ...
          <button type="submit" disabled={loading}
            className="w-full py-3 rounded-[var(--radius-sm)] text-sm font-bold disabled:opacity-40"
            style={{ background: 'linear-gradient(135deg, var(--gold) 0%, var(--gold-dark) 100%)', color: 'var(--bg-deep)' }}>
            {loading ? "登录中…" : "登录"}
          </button>
        </form>
      </div>
